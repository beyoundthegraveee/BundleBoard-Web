...
import { GetRandomCollectionsDocument } from '@/graphql/generated'
...
export function RandomCollectionGrid() {
  const { status } = useSession()
  
  const { data, loading, error, refetch } = useQuery(GetRandomCollectionsDocument, {
    variables: {
      limit: LIMIT
    },
    skip: status === "loading",
  ...
  const collections = (data?.getRandomCollections || []) as Collection[];
  ...
  if (loading) return (
    <div className="flex flex-col justify-center items-center py-40 space-y-3 font-sans">
      ...
      <span className="text-muted-foreground text-[11px] uppercase tracking-[0.2em] font-medium">
        Loading discover pool...
      </span>
    </div>
  )
  ...
  if (error) return (
    <div className="p-8 border border-destructive/20 text-destructive font-sans text-xs uppercase tracking-wider bg-destructive/5 flex flex-col items-start gap-4 rounded-none max-w-xl mx-auto m-4">
      <div>
        <span className="font-bold mr-2">Discovery stream error:</span> 
        {error.message || "FAILED_TO_STREAM_RANDOM_DATA"}
      </div>
      <button 
        onClick={() => refetch()}
