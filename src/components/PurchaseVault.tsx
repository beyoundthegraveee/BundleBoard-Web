              ...
              {purchase.items?.map((item: any) => (
                <div key={item.id} className="group border border-border/40 bg-card rounded-none shadow-sm overflow-hidden flex flex-col justify-between relative">
                  <div className="aspect-video border-b border-border/30 overflow-hidden bg-muted relative">
                    <img src={item.asset?.previewImage?.filePath} alt={item.asset?.name} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity duration-300" />
                  </div>
                  ...
                  <div className="p-4 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <div className="text-[9px] bg-muted border border-border/60 px-1.5 py-0.5 font-semibold uppercase text-muted-foreground tracking-wider shrink-0">{purchase.status}</div>
                    </div>
